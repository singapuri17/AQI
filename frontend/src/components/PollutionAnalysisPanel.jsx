...
function SourceTooltip({ tooltip }) {
  ...
  return (
    <span className="relative inline-flex items-center ml-1" style={{ zIndex: 1 }}>
      <InformationCircleIcon
        className="w-3.5 h-3.5 text-gray-500 hover:text-gray-300 cursor-help"
        onMouseEnter={() => setShow(true)}
        onMouseLeave={() => setShow(false)}
      />
      {show && (
        <span className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 bg-gray-800 border border-gray-600 rounded-lg p-2.5 text-xs text-gray-200 shadow-xl leading-relaxed pointer-events-none whitespace-normal">
          {tooltip}
    ...
    <div className="glass-card p-5 space-y-4">
      ...
      <div>
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
          AI Pollution Source Filter
          {loadedWards.length > 0 && pctLoaded < 100 && (
            <span className="ml-2 text-gray-600 font-normal normal-case">({pctLoaded}% loaded…)</span>
          )}
        </p>
        <div className="flex flex-wrap gap-2" style={{ position: 'relative', zIndex: 0 }}>
          {SOURCES.map(src => (
            <button
              key={src.id}
              type="button"
              onClick={() => { console.log('Filter clicked:', src.id); onFilterChange(src.id) }}
              style={activeFilter === src.id ? { backgroundColor: src.color + '33', borderColor: src.color, position: 'relative', zIndex: 1 } : { position: 'relative', zIndex: 1 }}
              className={clsx(
            ...
            >
              ...
              {src.id !== 'All' && activeFilter === src.id && loadedWards.length > 0 && (
                <span className="ml-0.5 px-1.5 py-0.5 rounded-full text-xs font-bold"
                  style={{ backgroundColor: src.color + '33', color: src.color }}>
                  {matchedWards.length}
                </span>
              )}
              ...
              <div className="bg-gray-800/40 rounded-xl p-4 border border-gray-700/30">
                ...
                {pieData.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-40 gap-2">
                    ...
                    <p className="text-xs text-gray-500">Loading ward data…</p>
                  </div>
                )}
